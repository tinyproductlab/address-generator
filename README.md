...
关注公众号，获取更多免费实用工具和产品更新。

<img src="assets/wechat-official-account-qr.png" width="180" height="180" alt="微信公众号：小产品实验室二维码">
...
- 美国新增 Delaware、Montana、Oregon 共 9 座城市，支持免税州筛选、固定城市及批量生成；筛选可随链接保留。
- 城市邮编与区县绑定，地图仍是城市参考坐标，不代表随机门牌的位置。
- 地图支持粘贴 `纬度, 经度`，展示原始坐标并复制。附近地址查询不会覆盖随机人物资料。
- 坐标查地址需要部署 [后端接口](server/README.md) 并配置供应商；当前使用现有后端转发 Photon 公开服务，无需账号；用量受公共服务限制。查询由用户点击触发，不参与批量生成。
- 新界面文案提供简体中文，其他语言暂以英语回退。
